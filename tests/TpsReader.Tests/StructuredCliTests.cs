...
public sealed class StructuredCliTests
{
    ...
    public void Query_supports_lossless_decimal_date_time_group_array_and_text_operations()
    {
        ...
        var table = new TpsTable(
            ...
            [
                Record(1, new Dictionary<string, object?>
                {
                    ...
                    ["T:GROUP"] = new TpsGroupValue("Alpha   ", [0x01, 0xA4]),
                ...
                Record(2, new Dictionary<string, object?>
                {
                    ...
                    ["T:GROUP"] = new TpsGroupValue("Other   ", [0x00, 0x00]),
        ...
        var request = Request(
            ...
            new PredicateInput("GROUP", "eq", "alpha"),
    ...
    [Theory]
    [InlineData("eq", "alpha beta", 1)]
    [InlineData("ne", "alpha beta", 2)]
    [InlineData("contains", "pha", 2)]
    [InlineData("starts-with", "alpha", 2)]
    [InlineData("ends-with", "beta", 1)]
    [InlineData("is-null", null, 1)]
    [InlineData("is-not-null", null, 3)]
    public void Group_predicates_use_trimmed_text_semantics(string op, string? literal, int expected)
    {
        var field = Field(1, "T:GROUP", TpsFieldType.Group);
        var table = new TpsTable(
            1,
            "T",
            [field],
            ...
            [
                Record(1, new Dictionary<string, object?>
                {
                    [field.Name] = new TpsGroupValue("Alpha Beta   ", [1])
                }),
                Record(2, new Dictionary<string, object?>
                {
                    [field.Name] = new TpsGroupValue("Alphabet   ", [2])
                }),
                Record(3, new Dictionary<string, object?>
                {
                    [field.Name] = new TpsGroupValue("Other   ", [3])
                }),
                Record(4, new Dictionary<string, object?> { [field.Name] = null })
            ]);

        var result = RecordQuery.Execute([table], Request(new PredicateInput("GROUP", op, literal)));

        Assert.Equal(expected, result.MatchedCount);
    }

    [Fact]
    public void Group_predicates_preserve_case_sensitivity_and_support_array_selectors()
    {
        var field = Field(1, "T:GROUPS", TpsFieldType.Group, elementCount: 2);
        var table = new TpsTable(
            1,
            "T",
            [field],
            ...
            [
                Record(1, new Dictionary<string, object?>
                {
                    [field.Name] = new object?[]
                    {
                        new TpsGroupValue("First   ", [1]),
                        new TpsGroupValue("Value Two   ", [2])
                    }
                })
            ]);

        Assert.Equal(1, RecordQuery.Execute(
            [table],
            Request(new PredicateInput("GROUPS[2]", "ends-with", "two"))).MatchedCount);
        Assert.Equal(0, RecordQuery.Execute(
            [table],
            Request(caseSensitive: true, new PredicateInput("GROUPS[2]", "ends-with", "two"))).MatchedCount);
    }
    ...
    public void Json_encoding_preserves_machine_types_and_blob_metadata()
    {
        ...
        var record = new TpsRecord(
            ...
            new Dictionary<string, object?>
            {
                ...
                ["T:GROUP"] = new TpsGroupValue("AB  ", [0, 0]),
        ...
        Assert.Equal("AB  ", values.GetProperty("T:GROUP").GetString());
